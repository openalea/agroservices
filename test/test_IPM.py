...
def test_get_schema_weatherdata(ipm):
    if ipm_ok:
        res = ipm.get_schema_weatherdata()
        assert type(res) is dict


def test_post_schema_weatherdata_validate(ipm):
    if ipm_ok:
        res = ipm.post_schema_weatherdata_validate(
            jsonfile=datadir + 'weather_data.json')
        assert type(res) is dict
        assert res["isValid"] == True


def test_get_schema_fieldobservation(ipm):
    res = ipm.get_schema_fieldobservation()
    assert type(res) is dict


def test_get_schema_modeloutput(ipm):
    res = ipm.get_schema_modeloutput()
    assert res is not None
    assert type(res) is dict


def test_post_schema_modeloutput_validate(ipm):
    res = ipm.post_schema_modeloutput_validate(
        jsonfile=datadir + 'modeloutput.json')
    assert type(res) is dict
    assert res['isValid'] == True
...
def test_get_weatheradapter(ipm):
    ...
    assert type(res) is dict
    assert all(key in res for key in (
        'timeStart', 'timeEnd', 'interval', 'weatherParameters',
        'locationWeatherData'))
    assert all(
        var in res['weatherParameters'] for var in [1002, 2001, 3002, 3101])
...
def test_get_weatherdatasource(ipm):
    res = ipm.get_weatherdatasource()
    assert type(res) is dict
    assert keys_exists(res[next(iter(res))], (
        'name', 'description', 'public_URL', 'endpoint', 'needs_data_control',
        'access_type', 'temporal', 'parameters', 'spatial'))


def test_get_weatherdatasource_location_point(ipm):
    res = ipm.get_weatherdatasource_location_point(latitude=59.678835236960765,
                                                   longitude=12.01629638671875,
                                                   tolerance=0)
    assert type(res) is list
    assert keys_exists(res[0], (
        'id', 'name', 'description', 'public_URL', 'endpoint',
        'authentication_type', 'needs_data_control', 'access_type', 'priority',
        'temporal', 'parameters', 'spatial', 'organization', 'active'))


def test_post_weatherdatasource_location(ipm):
...
def test_get_crop(ipm):
    res = ipm.get_crop()
    assert type(res) is list


def test_get_cropCode(ipm):
    res = ipm.get_cropCode(cropCode="DAUCS")
    assert type(res) is dict
    item = next(iter(res.values()))
    assert keys_exists(item, (
        'models', 'id', 'version', 'name', 'url', 'languages', 'organization'))
    item = next(iter(item['models'].values()))
    assert 'DAUCS' in item['crops']


def test_get_dss(ipm):
    res = ipm.get_dss()
    assert type(res) is dict
    item = next(iter(res.values()))
    assert keys_exists(item, (
...
def test_get_model(ipm):
    res = ipm.get_model(DSSId='no.nibio.vips', ModelId='PSILARTEMP')
    assert type(res) is dict
    assert keys_exists(res.keys(), (
        'name', 'id', 'version', 'type_of_decision', 'type_of_output',
        'description_URL', 'description', 'citation', 'keywords', 'pests',
        'crops',
        'authors', 'execution', 'input', 'valid_spatial', 'output'))
    assert res['id'] == 'PSILARTEMP'


def test_get_pest(ipm):
    res = ipm.get_pest()
    assert type(res) is list


def test_get_pestCode(ipm):
...
def test_get_dss_location(ipm):
    res = ipm.get_dss_location_point(latitude=59.67883523696076,
                                     longitude=12.01629638671875)
    assert type(res) is dict
    item = next(iter(res.values()))
    assert keys_exists(item, (
...
def test_post_dss_location(ipm):
