...
def test_get_weatherdatasource_location_point():
    ipm = IPM()
    res = ipm.get_weatherdatasource_location_point(
...
def test_post_weatherdatasource_location():
    ...
    res = ipm.post_weatherdatasource_location(
        tolerance=0, geoJsonfile=datadir + "GeoJson.json"
    )
    assert type(res) is list
    assert keys_exists(
        res[0].keys(),
...
def test_get_cropCode():
    ...
    assert keys_exists(
        item, ("models", "id", "version", "name", "url", "languages", "organization")
    )
...
def test_get_dss_location():
    ipm = IPM()
    res = ipm.get_dss_location_point(
        latitude=59.67883523696076, longitude=12.01629638671875
    )
    assert type(res) is dict
    item = next(iter(res.values()))
    assert keys_exists(
