...
def fix_get_weatherdatasource(resp):
    ...
    resp["ie.gov.data"]["endpoint"] = (
        "{WEATHER_API_URL}/rest/weatheradapter/meteireann/"
    )
    return resp
    ...
    if dss == "adas.datamanipulation":
        if model["id"] == "LeafWetnessDuration_RH":
            ...
            model["execution"]["input_schema"] = {
                "type": "object",
                "required": ["RH"],
                "properties": {"RH": {"type": "number", "minimum": 0, "maximum": 100}},
            }
            model["execution"]["endpoint"] = model["execution"]["endpoint"][:-8]
    if dss == "adas.dss":
        if model["id"] == "MELIAE":
            # add valid default and valid boundaries for growthStage
            model["execution"]["input_schema"]["properties"]["growthStage"][
                "default"
            ] = 55
            model["execution"]["input_schema"]["properties"]["growthStage"]["type"] = (
                "integer"
            )
            model["execution"]["input_schema"]["properties"]["growthStage"][
                "minimum"
        ...
        if model["id"] == "DEROAG_Cereals":
            ...
            model["execution"]["input_schema"]["properties"]["configParameters"][
                "properties"
            ]["observationClass"]["minItems"] = 1
    if dss == "no.nibio.vips":
        if model["id"] == "PSILAROBSE":
            # field obs object misses type
            model["execution"]["input_schema"]["definitions"]["fieldObs_PSILRO"][
                "type"
            ] = "object"
        if model["id"] == "DELIARFOBS":
            # field obs object misses type
            model["execution"]["input_schema"]["definitions"]["fieldObs_HYLERA"][
                "type"
            ] = "object"
            model["execution"]["input_schema"]["definitions"]["fieldObs_HYLERA"][
                "required"
            ...
            props = model["execution"]["input_schema"]["properties"][
                "configParameters"
            ]["properties"]
            quantifications = props.pop("fieldObservationQuantifications")
            props["fieldObservations"] = {
                ...
                "items": {
                    ...
                    "properties": {
                        ...
                        "quantification": quantifications["items"]["oneOf"][0],
                    },
                },
            }
        if model["id"] == "SEPAPIICOL":
            # field obs object misses type
            model["execution"]["input_schema"]["definitions"]["fieldObs_SEPTAP"][
                "type"
            ] = "object"
            model["execution"]["input_schema"]["definitions"]["fieldObs_SEPTAP"][
                "required"
        ...
        if model["id"] == "BREMIALACT":
            ...
            for w in ("start", "end"):
                model["input"]["weather_data_period_" + w][0]["determined_by"] = (
                    "FIXED_DATE"
                )
            # end point is wrong, bug has been reported, to be check in newer version
            model["execution"]["endpoint"] = (
                "https://coremanager.vips.nibio.no/models/BREMIALACT/run/ipmd"
            )
    elif dss == "dk.seges":
        # 'weatherData' is misspelled
        if "WeatherData" in model["execution"]["input_schema"]["properties"]:
            model["execution"]["input_schema"]["properties"]["weatherData"] = model[
                "execution"
            ]["input_schema"]["properties"].pop("WeatherData")
