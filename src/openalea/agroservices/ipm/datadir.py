...
def postman_tests():
    ...
    with open(
        datadir + "IPM Decisions Weather API tests.postman_collection.json"
    ) as jsonfile:
        postman = json.load(jsonfile)
    adapters = {
        it["name"]: it
        for it in [
            item for item in postman["item"] if item["name"] == "WeatherAdapterService"
        ][0]["item"]
    }
...
def country_mapping():
    ...
    """
    with open(datadir + "countries.json") as f:
        mapping = json.load(f)
    return mapping
