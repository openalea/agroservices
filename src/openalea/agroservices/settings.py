...
defaultParams = {
    "user.email": [
        "unknown",
        str,
        "email address that may be used in some utilities (e.g. EUtils)",
    ],
...
class ConfigReadOnly:
                    ...
                    if isinstance(value, cast) is True:
                        ...
                        print(
                            "Warning:: found an incorrect type while parsing {} file. In section '{}', the option '{}' should be a {}. Found value {}. Trying a cast...".format(
                                self.user_config_file_path, section, key, cast, value
                            )
                        )
    ...
    config_file = property(_get_config_file, doc="config filename (without path)")

    def init(self):
        """Reads the user_config_file and update params.
        Creates the directories for config and cache if they do not exist
...
class AgroServicesConfig(ConfigReadOnly):
    def __init__(self):
        super(AgroServicesConfig, self).__init__(
            name="agroservices", default_params=defaultParams
        )
